...
编辑器是 CodeMirror 6，直接在 markdown 源码上写；阅读态始终是服务端渲染的
HTML（无 JS 可读、可缓存），编辑态只是临时盖在它上面。CodeMirror 那一包
（约 500 KB）只在点下铅笔后才动态加载，匿名访客永远拿不到。
