...
| 登录   | 单口令                      | 首次登录设置（settings 表存哈希），HMAC 签名 cookie         |
...
> 口令哈希的迭代次数沿用 100000：哈希串自带迭代次数，改默认值不影响已有口令；对单用户站点，真正拦人的是限流加二十位以上的随机口令。
...
会话是一个签名 cookie，不落库：单用户站点不需要 session 表，能证明「你输对过口令」就够了，所以载荷里只有过期时间，没有身份。代价是签发后无法主动吊销，有效期压到 7 天；签名密钥存在 `settings.session_secret`（首次启动自动生成），真要立刻失效就删掉那一行让所有会话作废，然后重启。
