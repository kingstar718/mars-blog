...
const run = async () => {
  ...
  for (const item of [
    ...
    statements.push(
      `INSERT INTO entries (kind, title, body, pub_datetime, status, ai_generated, created_at, updated_at)
       VALUES (${sql(item.kind)}, ${isPost ? sql(data.title) : "NULL"}, ${sql(body)}, ${sql(pub)}, 'published', ${data.aiGenerated === undefined ? "NULL" : data.aiGenerated ? 1 : 0}, ${sql(pub)}, ${sql(pub)});`
    );
