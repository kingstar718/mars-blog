...
排版、字体和阅读体验一路沿袭下来。v3 把「打开页面就能写」搬回了静态架构：
登录之后，阅读页面本身就是编辑界面。
...
| 写作   | CodeMirror 6 就地编辑，贴图/拖图即压缩上传（浏览器端 400/800/1600 三档 webp+jpeg）   |
...
2. D1 建库并应用迁移：`pnpm dlx wrangler d1 migrations apply <库名> --remote`
   （需要本地 `wrangler.toml`，已 gitignore）
