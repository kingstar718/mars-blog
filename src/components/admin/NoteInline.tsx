 ...
 * 随记的就地编辑。
 *
 * 随记没有详情页，写一条随记本来就是「打开、敲两行、发出去」，
 * 为它跳一趟后台再跳回来太重了。
 *
 * 编辑器通过 portal 渲染进那条随记自己的位置（服务端在每条随记后面留了一个
 * 空占位 div），同时把渲染好的正文藏起来——所以视觉上是这条随记原地变成了
 * 编辑态，而不是从别处弹出来一个面板。
 *
 * 两个状态之间是交叉淡入淡出，不是硬切：正文淡出之后编辑框才淡入，
 * 编辑框的最小高度直接取自原正文的高度，所以下面的内容不会跳。
 *
 * 岛本身不负责渲染随记正文：正文是服务端渲染的 HTML，保持无 JS 可读、可缓存。
 */
...
export default function NoteInline() {
  ...
  useEffect(() => {
    const onEdit = (event: Event) => {
      ...
      // 但要封顶——图多的随记在编辑态只是几行 markdown 链接，
      // 照搬阅读态的高度会留下一大片空白
