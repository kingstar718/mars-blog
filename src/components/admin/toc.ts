 ...
 * 编辑态接管页面上的目录。
 *
 * 阅读态的目录是服务端用 headings_json 渲染的，锚点指向正文里的标题。
 * 进编辑态之后正文被 hidden 掉，那些标题没有布局盒，浏览器无从滚动——
 * 点了只会把 hash 写进地址栏，看着像坏了。而且那份目录本身也过时：
 * headings_json 是**发布时**算的，编辑期间新加的章节不在里面。
 *
 * 所以编辑态换一份：内容实时从编辑器里算（见 outline.ts），点击滚编辑器
 * 对应的行。退出时把原来那份原样放回去——它仍然是发布内容的正确目录。
 *
 * 只换 <ul>，外面的容器、图钉按钮、悬停展开的 CSS 全部照旧。
 */
...
const sidebarItem = (item: OutlineItem, onPick: () => void) => {
  ...
  const link = h(
    "a",
    {
      // 保留 <a>：容器的展开靠 :focus-within，换成非可聚焦元素就不展开了。
      // href 给 # 只为可聚焦，点击一律 preventDefault
      href: "#",
      class:
...
export const takeoverToc = (editor: EditorHandle) => {
  ...
  for (const [selector, kind] of [
    ...
    const live = h("ul");
    // parent 必须在 replaceChild 之前取：换完 original 就脱离文档了，
    // 那时候再读 original.parentElement 拿到的是 null
    parent.replaceChild(live, original);
    slots.push({ parent, original, live, kind });
  }
  // 页面上没有目录容器（比如刚建的空文章）就什么都不做
  if (slots.length === 0) return { refresh: () => {}, restore: () => {} };

  const sidebar = document.getElementById("toc-sidebar");
  // 已发布版本本来就不够两条时，服务端渲染的就是 empty 状态。
  // 退出编辑态要还原成它，不能一律清掉——否则会冒出一个空目录
  ...
  const render = () => {
    ...
    // 与服务端同一条规则：少于两条不出目录。整篇没有标题时侧栏就该消失，
    // 而不是留一个空盒子在右边
